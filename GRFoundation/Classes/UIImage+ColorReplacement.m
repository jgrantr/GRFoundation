...
MAKE_CATEGORIES_LOADABLE(UIImage_ColorReplacement);
...
+ (RGBAColor) colorsFromColor:(UIColor *)color {
...
+ (UIImage *) imageWithSize:(CGSize)size color:(UIColor *)color {
	return [self imageWithSize:size cornerRadius:0 color:color];
}
...
- (UIImage *) maskWithImage:(UIImage *)maskImage andUnderlayImage:(UIImage *)underlay {
	UIImage *newImage = [self maskWithImage:maskImage];
	return [UIImage overlayImage:newImage onTopOfImage:underlay];
}
...
- (UIImage *)imageByRemovingColorsWithMinColor:(UIColor *)minColor maxColor:(UIColor *)maxColor {
	return [self imageByReplacingColorsWithMinColor:minColor maxColor:maxColor withColor:[UIColor colorWithRed:0 green:0 blue:0 alpha:0]];
}
...
- (UIImage *)imageByReplacingColorsWithMinColor:(UIColor *)minColor maxColor:(UIColor *)maxColor withColor:(UIColor *)newColor {
    ...
    float height = CGImageGetHeight(imageRef);
    CGRect bounds = CGRectMake(0, 0, width, height);
	RGBAColor minColors = [UIImage colorsFromColor:minColor];
	RGBAColor maxColors = [UIImage colorsFromColor:maxColor];
	RGBAColor newColors = [UIImage colorsFromColor:newColor];
